...
class SAFFRON_proc_batch:
    def __init__(self, alpha0, numhyp, lbd, gamma_vec_exponent, markov_lag):
    ...
    def run_fdr(self, pvec):
        ...
        for k in range(0, numhyp):
            ...
            if self.wealth_vec[k] > 0:
                ...
                candidates_total = sum(candidates[0:k+1-self.markov_lag])
                zero_gam = self.gamma_vec[k + 1 - (int)(candidates_total)]
                # Update alpha_t
                if len(last_rej) > 0:
                    if last_rej[0]<= (k+1 - self.markov_lag):
                        candidates_after_first = sum(candidates[last_rej[0]+1:k+1-self.markov_lag])
                    ...
                    if len(last_rej) >= 2:
                        ...
                        indic = last_rej<(k+1-self.markov_lag)
