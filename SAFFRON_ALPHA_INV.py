...
class SAFFRON_ALPHA_INV_proc_batch:
    ...
    def __init__(self, alpha0, numhyp, gamma_vec_exponent):
        ...
        self.w0 = self.alpha0/4 # initial wealth, has to satisfy w0 < (1 - lbd)*alpha0
        ...
        self.lbd = self.alpha[1] # candidate threshold lambda
    ...
    def run_fdr(self, pvec):
        ...
        for k in range(0, numhyp):
            ...
            if self.wealth_vec[k] > 0:
                ...
                zero_gam = self.gamma_vec[k + 1 - candidates_total.astype(int)]
                # Update alpha_t
                if len(last_rej) > 0:
                    if len(last_rej) > 1:
                        candidates_after_first = sum(candidates[last_rej[0] + 1:])
                        first_gam = self.gamma_vec[k + 1 - last_rej[0] - candidates_after_first.astype(int)]
                    else:
                        first_gam = 0
                    if len(last_rej) > 2:
                        sum_gam = self.gamma_vec[
                            (k + 1) * np.ones(len(last_rej) - 1, dtype=int) - last_rej[1:] - self.count_candidates(
                                last_rej, candidates)]
                        sum_gam = sum(sum_gam)
                    else:
                        sum_gam = 0
