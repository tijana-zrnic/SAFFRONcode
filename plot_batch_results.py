...
def plot_results(plot_style, whichrun, FDRrange, pirange, hyprange, mu_gap, sigma, NUMHYP, num_runs, mod_choice, NUMDRAWS = 1):
    ...
    if plot_style == 0:
        ...
        for FDR_j, FDR in enumerate(FDRrange):
            ...
            filename_pre = 'MG%.1f_Si%.1f_FDR%d_NH%d_ND%d_PM%.2f_NR%d_L%d_MOD%d' % (mu_gap, sigma, FDR, NUMHYP, NUMDRAWS, pi, num_runs, mod_choice)
        ...
        for index, FDR in enumerate(FDRrange):
            ...
            filename_pre = 'MG%.1f_Si%.1f_FDR%d_NH%d_ND%d' % (mu_gap, sigma, FDR, NUMHYP, NUMDRAWS)
            all_filenames = [filename for filename in os.listdir('./dat') if filename.startswith(filename_pre)]
