...
def plot_results(plot_style, whichrun, FDRrange, pirange, hyprange, mu_gap, sigma, NUMHYP, num_runs, mod_choice, NUMDRAWS = 1):
    ...
    if plot_style == 0:
        ...
        for FDR_j, FDR in enumerate(FDRrange):
            ...
            filename_pre = 'MG%.1f_Si%.1f_FDR%d_NH%d_ND%d_PM%.2f_NR%d_MOD%d' % (mu_gap, sigma, FDR, NUMHYP, NUMDRAWS, pi, num_runs, mod_choice)
        ...
        #### Wealth vs HYP ####
        filename = 'WealthvsHP_Plot%d_MG%.1f_Si%.1f_NH%d_ND%d_PM%.2f_HR%d_R%d' %  (plot_style, mu_gap, sigma, NUMHYP, NUMDRAWS, pi, hyplen, whichrun)
        plot_curves_mat(xs, wealth_mat, legends_list, plot_dirname, filename,  'Hypothesis index', 'Wealth($J$)', 0, leg_col = leg_col)

        #### alpha vs. HYP ####
        filename = 'alphavsHP_MG%.1f_Si%.1f_NH%d_ND%d_PM%.2f_HR%d_R%d' %  (mu_gap, sigma, NUMHYP, NUMDRAWS, pi, hyplen, whichrun)
        ...
        for ind, FDR in enumerate(FDRrange):

            filename_pre = 'MG%.1f_Si%.1f_FDR%d_NH%d_ND%d_' % (mu_gap, sigma, FDR, NUMHYP, NUMDRAWS)
            all_filenames = [filename for filename in os.listdir('./dat') if filename.startswith(filename_pre)]
        ...
        filename = 'FDRvsPI_MG%.1f_Si%.1f_NH%d_ND%d_MOD%d' %  (mu_gap, sigma, NUMHYP, NUMDRAWS, mod_choice)
        plot_errors_mat(xs, FDR_av, FDR_std, legends_list, plot_dirname, filename, x_label, 'FDR')

        ##### TDR vs pi ####
        filename = 'PowervsPI_MG%.1f_Si%.1f_NH%d_ND%d_MOD%d' %  (mu_gap, sigma, NUMHYP, NUMDRAWS, mod_choice)
